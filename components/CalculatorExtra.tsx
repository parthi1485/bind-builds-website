'use client';
import { compoundWallArea, extraAmount, initialExtra, parkingArea, type ExtraOption, type ExtraSelection } from '@/lib/calculator-options';
...
export default function CalculatorExtra({item,value:stored,onChange}:Props){
 ...
 const setCompoundDimension=(field:'length'|'height',next:string)=>{
  const length=field==='length'?next:(value.length??'');
  const height=field==='height'?next:(value.height??'');
  onChange({[field]:next,quantity:compoundWallArea(length,height),mode:'unit'});
 };
 return <div className={`calcExtra ${value.selected?'selected':''}`}>
  ...
  {value.selected&&<div className="extraEditor">
   ...
   <label>Pricing method<select value={value.mode} onChange={e=>onChange({mode:e.target.value as ExtraSelection['mode']})}>{item.key!=='demolition'&&<option value="unit">Quantity × unit rate</option>}<option value="lump">Enter total allowance</option><option value="unpriced">Request a quote</option></select></label>
   {value.mode==='unit'&&<>
    {item.quantityPresets&&<fieldset className="calcChoices"><legend>Suggested capacity</legend><div className="calcPresets">{item.quantityPresets.map(preset=><button type="button" key={preset.label} aria-pressed={value.quantity===String(preset.quantity)} onClick={()=>onChange({quantity:String(preset.quantity),mode:'unit'})}>{preset.label}<small>{preset.note??`${preset.quantity.toLocaleString('en-IN')} ${item.unit}`}</small></button>)}</div><p className="calcHint">Family-size presets are for early budgeting only. Final storage capacity should be confirmed for actual occupancy and water availability.</p></fieldset>}
    {item.key==='compound'?<div className="extraUnitFields compoundWallFields">
      <label>Wall length <span>ft</span><input type="number" inputMode="decimal" min="0.01" max="10000" step="0.01" value={value.length??''} onChange={e=>setCompoundDimension('length',e.target.value)}/></label>
      <label>Average height <span>ft</span><input type="number" inputMode="decimal" min="0.01" max="100" step="0.01" value={value.height??''} onChange={e=>setCompoundDimension('height',e.target.value)}/></label>
      <label>Calculated wall area <span>sq.ft</span><input type="number" readOnly tabIndex={-1} value={value.quantity}/></label>
      <label>Unit rate <span>₹ / sq.ft</span><input type="number" inputMode="decimal" min="0.01" max="10000000" step="0.01" value={value.rate} onChange={e=>onChange({rate:e.target.value})}/></label>
     </div>:<div className="extraUnitFields"><label>{item.key==='parking'?'Parking area':'Quantity'} <span>{item.unit}</span><input type="number" inputMode="decimal" min="0.01" max="100000" step="0.01" placeholder="To be confirmed" value={value.quantity} onChange={e=>onChange({quantity:e.target.value,...(item.key==='parking'?{cars:0}:{})})}/></label><label>Unit rate <span>₹ / {item.unit}</span><input type="number" inputMode="decimal" min="0.01" max="10000000" step="0.01" placeholder="Enter rate" value={value.rate} onChange={e=>onChange({rate:e.target.value})}/></label></div>}
