...
import { calculateEmi, compactMoney, configuration, floorName, money, splitStages, validNumber, type Estimate, type EstimateInput } from '@/lib/calculator';
...
export default function CalculatorReport({ input, estimate, packageIndex, headingRef, onEdit }: Props) {
  ...
  const download = async () => {
    ...
    try {
      const data: PdfReportData = { packageName: selected.name, total: money(estimate.total), date: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
        ...
        rows, unpriced: estimate.unpriced.map(item => item.label), comparisons: comparisons.map(item => [item.name, `${money(item.rate)} / sq.ft`, money(item.base)]),
  ...
  return <div className="calcReport">
    <div className="calcReportHero">
      <div><span className="eyebrow">Your first step towards a well-planned home</span><h2 ref={headingRef} tabIndex={-1}>A clearer picture<br />of your budget.</h2><p>{selected.name} · {configuration(input.floors.length)} · {estimate.area.toLocaleString('en-IN')} sq.ft calculated area</p></div>
      <div className="calcReportTotal"><span>Planning subtotal</span><strong>{compactMoney(estimate.total)}</strong><span>{money(estimate.total)}</span><p>Taxes, approvals and other exclusions are additional. Subject to a project-specific proposal.</p></div>
    </div>
    ...
    <section className="calcReportCard calcCompare"><span className="eyebrow">02 / Same area. Different specifications.</span><h3>Compare your construction base.</h3><p className="calcHint">Comparison uses the same {estimate.area.toLocaleString('en-IN')} sq.ft. Extras and reserve are excluded so you can compare base rates clearly.</p><div className="calcComparisonGrid">{comparisons.map((item, index) => <article key={item.name} className={packageIndex === index ? 'chosen' : ''}><span>{item.name}{packageIndex === index && <small>Selected</small>}</span><strong>{compactMoney(item.base)}</strong><p>{money(item.rate)} / sq.ft</p><Link href={`/packages#${packages[index].key}`} target="_blank" rel="noopener noreferrer">View specification ↗</Link></article>)}</div></section>
    <section className="calcReportCard"><span className="eyebrow">03 / An illustrative allocation</span><h3>See the build in stages.</h3><p className="calcHint">One way to distribute the {money(estimate.base)} base construction budget for planning. These assumed percentages are not a BOQ or a payment schedule.</p><div className="calcStageGrid">{allocation.map((item, index) => <div className="calcStage" key={item.name}><span className="calcStageNumber">{String(index + 1).padStart(2, '0')}</span><div><span>{item.name}</span><div className="calcStageTrack"><i style={{ width: `${item.percent * 4}%` }} /></div></div><strong>{money(item.amount)}<small>{item.percent}% of base</small></strong></div>)}</div></section>
