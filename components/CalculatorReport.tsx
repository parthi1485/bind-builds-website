...
export default function CalculatorReport({ input, estimate, packageIndex, headingRef, onEdit }: Props) {
  ...
  if (input.headroom) rows.push([`Separate headroom · ${input.headroom.toLocaleString('en-IN')} sq.ft × ${money(estimate.headroomRate)}/sq.ft`, money(estimate.headroomCost)]);
  ...
  const notes = [
    ...
    input.headroom ? `Separate headroom: ${input.headroom} sq.ft × ${money(estimate.headroomRate)}/sq.ft = ${money(estimate.headroomCost)}` : '',
  ...
  const assumptions = [
    ...
    `Floor areas are combined at the chosen package rate. Any separate headroom is added once at its editable headroom rate (default ${money(estimate.headroomRate)}/sq.ft for this estimate). Final area measurement and treatment of parking, headroom and open spaces must be agreed.`,
  ...
  const download = async () => {
    ...
    try {
      const data: PdfReportData = { packageName: selected.name, total: money(estimate.total), date: new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
        ...
        facts: [['Plot', `${input.plot.toLocaleString('en-IN')} sq.ft`], ['Configuration', configuration(input.floors.length)], ['Calculated area', `${estimate.area.toLocaleString('en-IN')} sq.ft`], ['Package rate', `${money(input.rate)} / sq.ft`], ...(input.headroom ? [['Headroom rate', `${money(estimate.headroomRate)} / sq.ft`]] as [string,string][] : [])],
  ...
  return <div className="calcReport">
    ...
    <div className="calcReportColumns">
      <section className="calcReportCard"><span className="eyebrow">01 / Where the number comes from</span><h3>Your cost breakdown.</h3><p className="calcHint">{estimate.floorArea.toLocaleString('en-IN')} sq.ft at {money(input.rate)} / sq.ft{input.headroom ? ` + ${input.headroom.toLocaleString('en-IN')} sq.ft headroom at ${money(estimate.headroomRate)} / sq.ft` : ''}, plus selected additional items.</p><dl className="calcCostRows">{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}<div className="calcCostTotal"><dt>Planning subtotal</dt><dd>{money(estimate.total)}</dd></div></dl></section>
